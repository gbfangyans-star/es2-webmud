...
#ifdef SAVE_USER
    /* Neolith compatibility: assure_file() is a simul_efun, so master.c
     * validates mkdir using the player's caller identity. Permit only the
     * canonical first-letter save directories for that same player's euid.
     * This is safe for concurrent players because each mkdir target is
     * deterministic and contains no player-controlled path component. */
    if( func=="mkdir" && sizeof(path)==3
    &&  path[0]=="data"
    &&  (path[1]=="user" || path[1]=="login" || path[1]=="mail")
    &&  path[2]==euid[0..0] )
        return 1;
#endif
