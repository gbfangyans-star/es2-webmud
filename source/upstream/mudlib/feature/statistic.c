...
int health_regenerator (object me, string stat, int max, int heal, int cur) {
    ...
    // Slowly heal if the character is well fed.  CUSTOM A-H rule: 無腸
    // has no food statistic (0/0), so hunger must not block normal healing.
    if (userp(me) && me->query_race() != "woochan"
    &&  ((int)me->query_stat("food") < 1))
        return 0;
