...
int study_content(object me)
{
    ...
    /* CUSTOM HOME SYSTEM: a study room may improve study efficiency.
     * The room bonus scales learning points only; book requirements, caps,
     * stat costs and fatigue rules remain canonical.
     */
    if( environment(me) && environment(me)->query("custom_home/study_bonus") > 100 )
        point = point * environment(me)->query("custom_home/study_bonus") / 100;
    foreach(t, max in content) {
