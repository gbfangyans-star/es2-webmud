...
int main(object me, string arg) {
    ...
    me->supplement_stat("gin", me->query_stat_maximum("gin") * 35 / 100);
    me->supplement_stat("kee", me->query_stat_maximum("kee") * 35 / 100);
    me->supplement_stat("sen", me->query_stat_maximum("sen") * 35 / 100);
    me->set("custom_race/cooldown/radiate", time() + RADIATE_COOLDOWN);
    if (userp(me)) me->save();
    message_vision(HIW "$N身上泛起一陣柔和光芒，精氣神迅速回復。\n" NOR, me);
    return 1;
}
int help(object me) { write("指令格式：radiate\n恢復精、氣、神各自最大值的 35%，冷卻 2 tick。\n"); return 1; }
