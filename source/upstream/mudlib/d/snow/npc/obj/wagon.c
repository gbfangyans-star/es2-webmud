...
int do_load(string arg)
{
	...
	/*
	 * CUSTOM WebMUD: record how many crates were actually loaded.
	 * The core carry check already scales with STR via
	 * query_strength("carriage"), so stronger players can carry more crates.
	 */
	if( ob->id("crate") && (int)ob->query("wage") > 0 ) {
		this_player()->add_temp("wage_crates", 1);
